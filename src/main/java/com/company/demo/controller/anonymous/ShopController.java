...
public class ShopController {
    ...
    @PostMapping("/api/pay")
    public ResponseEntity<?> getPayment(@Valid @RequestBody CreateOrderReq orderReq){
        User user = ((CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUser();

        Order order = orderService.createOrder(orderReq, user.getId());
        String description = order.getId()+"-"+"Test";
        String response = paymentVNPayService.genUrl(order.getTotalPrice(), description);
    ...
    public String paymentSuccessfull(Model model,
        ...
        String trans_type = vnp_BankCode +"-"+ vnp_BankTranNo +"-"+ vnp_CardType;
        Long amount = Long.parseLong(vnp_Amount)/100;
        String []info = vnp_OrderInfo.split("-");
        Long orderId = Long.parseLong(info[0]);

        Transaction trans = new Transaction(trans_id, trans_type, orderId, amount, info[1]);
        paymentVNPayService.addTransaction(trans);
        ...
        model.addAttribute("date", trans.getDate().toString());
